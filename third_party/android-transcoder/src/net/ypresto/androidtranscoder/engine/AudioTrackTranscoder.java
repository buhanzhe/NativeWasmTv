...
public class AudioTrackTranscoder implements TrackTranscoder {
    ...
    private int drainDecoder(long timeoutUs) {
        ...
        if (mBufferInfo.size > 0) {
            ByteBuffer pcm = mDecoderBuffers.getOutputBuffer(result).duplicate();
            pcm.position(mBufferInfo.offset); pcm.limit(mBufferInfo.offset + mBufferInfo.size);
            mAudioChannel.drainDecoderBufferAndQueue(result, mBufferInfo.presentationTimeUs, pcm);
        } else mDecoder.releaseOutputBuffer(result, false);
        if ((mBufferInfo.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
            mIsDecoderEOS = true;
            mAudioChannel.drainDecoderBufferAndQueue(AudioChannel.BUFFER_INDEX_END_OF_STREAM, mBufferInfo.presentationTimeUs, null);
        }
