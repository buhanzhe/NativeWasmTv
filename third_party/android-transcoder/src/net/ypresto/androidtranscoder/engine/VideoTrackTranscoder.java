...
public class VideoTrackTranscoder implements TrackTranscoder {
    ...
    private long lastRenderedUs = Long.MIN_VALUE;
    ...
    public void setup() {
        ...
        mDecoderOutputSurfaceWrapper.setRotation(rotation);
        try {
    ...
    private int drainDecoder(long timeoutUs) {
        ...
        long frameInterval = 1000000L / Math.max(1, mOutputFormat.getInteger(MediaFormat.KEY_FRAME_RATE));
        boolean doRender = mBufferInfo.size > 0 && (lastRenderedUs == Long.MIN_VALUE
                || mBufferInfo.presentationTimeUs >= lastRenderedUs - 1000);
        if (doRender) {
            if (lastRenderedUs == Long.MIN_VALUE) lastRenderedUs = mBufferInfo.presentationTimeUs;
            do { lastRenderedUs += frameInterval; } while (lastRenderedUs <= mBufferInfo.presentationTimeUs);
        }
