...
public class QueuedMuxer {
    ...
    public interface StreamSink {
        void format(SampleType type, MediaFormat format);
        void sample(SampleType type, ByteBuffer data, MediaCodec.BufferInfo info);
    }
    private StreamSink streamSink;
    public QueuedMuxer(StreamSink sink) {
        mMuxer = null; mListener = null; mSampleInfoList = new ArrayList<>(); streamSink = sink;
    }
    ...
    public void setOutputFormat(SampleType sampleType, MediaFormat format) {
        if (streamSink != null) { streamSink.format(sampleType, format); return; }
        switch (sampleType) {
            case VIDEO:
                mVideoFormat = format;
                break;
    ...
    public void writeSampleData(SampleType sampleType, ByteBuffer byteBuf, MediaCodec.BufferInfo bufferInfo) {
        if (streamSink != null) { if (bufferInfo.size > 0) streamSink.sample(sampleType, byteBuf, bufferInfo); return; }
        if (mStarted) {
